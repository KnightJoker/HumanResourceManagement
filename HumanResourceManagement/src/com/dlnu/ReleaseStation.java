...
public class ReleaseStation extends JPanel {
	/**
	 * 
	 */
	...
	public ReleaseStation() {
		...
		setLayout(null);
		{
			JLabel labelpass = new JLabel("发 布 岗 位");
		...
		{
			JLabel label = new JLabel("招聘人数");
			label.setBounds(140, 170, 70, 17);
			add(label);
		}
		{
			JLabel lbli = new JLabel("有效期至：");
			lbli.setBounds(390, 170, 70, 17);
			add(lbli);
		}
		{
			JLabel label = new JLabel("职位类别：");
			label.setBounds(140, 220, 70, 17);
			add(label);
		}
		{
			JLabel label = new JLabel("工作地点：");
			label.setBounds(390, 220, 70, 17);
			add(label);
		}
		{
			JLabel label = new JLabel("岗位描述：");
			label.setBounds(140, 270, 70, 17);
			add(label);
		}
		{
			JLabel label = new JLabel("入职条件：");
			...
			button.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (textField.getText().equals("")
						...
						String sqlInsert = "insert into t_station values('"
								+ textField.getText() + "'," + "'"
								+ textField_4.getText() + "','"
								+ textField_2.getText() + "','"
								+ textField_6.getText() + "','"
